...
export const PaymentCard = ({
  ...
  return (
    <div className="text-black space-y-4 pb-2">
      ...
      <div
        className={`flex flex-col p-4 border ${
          discountAmount !== 0 ? `border-green-800` : `border-gray-300`
        } rounded-xl`}
      >
        <div className="flex flex-col pb-3 border-b">
          <div className="flex justify-between">
            <h1>
              Price ({totalPrice.length} item{totalPrice.length > 1 ? "s" : ""})
            </h1>
            <h1>₹{totalPrice.amount.toLocaleString("en-IN")}</h1>
          </div>
          {discountAmount !== 0 && (
            <div className="flex justify-between font-bold text-green-800">
              <h1>Discount ({discountAmount}%)</h1>
              <h1>
                - ₹{((discountAmount / 100) * totalPrice.amount).toLocaleString("en-IN")}
              </h1>
            </div>
          )}
          <div className="flex justify-between">
        ...
        <div className="flex justify-between pt-3 font-bold">
          <h1>Total Payment</h1>
          <h1>₹{totalPrice.discounted.toLocaleString("en-IN")}</h1>
        </div>
      </div>
      ...
      <div className="px-2">
        {paymentArr.map((data, i) => (
          <div key={i} className="flex items-center mb-4">
            <label
              htmlFor={data.id}
              className="flex w-full items-center text-sm font-medium text-gray-900 space-x-2"
            >
              <input
                id={data.id}
                type="radio"
                ...
                onChange={handleChange}
                name="payment-method"
                className="w-[18px] h-[18px] mr-2 text-green-600 bg-gray-100 border-gray-300 focus:ring-green-500 focus:ring-2"
              />
              {data.icon}
